Shader "Custom/CelShader"
{
    Properties
    {
        ...
        _ShadowBands("Shadow bands", Range(1,7)) = 1
    ...
    SubShader
    {
        ...
        half4 LightingCelShaded(SurfaceOutputCelShaded s, half3 lightDir, half3 viewDir, half atten) {
            ...
            half bandedLight = round(saturate(nDotL/_LightCutoff) * _ShadowBands) / _ShadowBands;

            float3 refl = reflect(normalize(lightDir), s.Normal);
            float dotRefl = dot(viewDir, -refl);
            float3 specular = _SpecularColor.rgb * step(1 - s.Smoothness, dotRefl);

            float3 rim = _RimColor * step(1 - _RimSize,1 - saturate(dot(normalize(viewDir), s.Normal)));

            half stepAtten = round(atten);
            half shadow = bandedLight * stepAtten;
